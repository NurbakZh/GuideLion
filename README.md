...
2. **O** standing for  **Open / Closed principle**. We tried to maximize the extendability of our classes without making any additional alters to it. For example, we have been using Core ViewControlers for main pages of our application, which helped us to minimize additional Views for certain situations. Moreover, we have used managers with static and global classes, which are general for all other classes and instances.
3. **L** standing for **Liskov Substitution**. If we derive the class from some base class, then this derived class should substitute the parent class without interrupting the behavior of the project. For example, all of the view controllers are child classes of UIViewController, and can substitute each other in case of derivation of one of the child classes.
4. **I** standing for **Interface Segregation principle**. We tried splitting up a larger interfaces into some smaller ones, so that each one is only responsible for a smaller scope of the features that it actually needs. All interfaces were divided to ViewControllers, Managers, Models in order to implement only important features for certain interface or its instance
5. **D** standing for **Dependency Inversion principle**. Relying on this principle, we made the low level modules be dependent on the higher level ones, on the abstraction, but not vice versa. For example HomeViewController, SearchViewController modules depend on UIViewController functionality and not vice versa
## 🧱 Architecture
